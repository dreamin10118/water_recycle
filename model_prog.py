...
from preprocessing_lib import get_velocity_acc_df
import warnings

warnings.filterwarnings("ignore")
# 讀取資料
mydata = pd.read_pickle('my_water_data.pkl')
# 選取要測試的預測日期
...
for day_i in sel_dates:
    ...
    t2flow_pred = pd.concat([t2flow_pred, obtain_best_pred(past_model_i, latest_model_i)], axis=0)
...
for day_i in sel_dates:
    ...
    outflow_pred  = pd.concat([outflow_pred, obtain_best_pred(past_model_i, latest_model_i)], axis=0)
...
for day_i in sel_dates:
    ...
    water_rate_pred  = pd.concat([water_rate_pred, obtain_best_pred(past_model_i, latest_model_i)], axis=0)
...
water_rate_pred.drop(['actual'], axis=1, inplace=True)
outflow_pred.rename(columns={'pred':'outflow_t2_pred'}, inplace=True)

outflow_pred.drop(['actual'], axis=1, inplace=True)
t2flow_pred.rename(columns={'pred':'t2flow_pred'}, inplace=True)

t2flow_pred.drop(['actual'], axis=1, inplace=True)
...
# print(pred_and_status.head())
pred_and_status.to_csv('result.csv', index=False)